...
void MSAC::fillDataContainers(std::vector<std::vector<cv::Point>> &lineSegments)
{
	...
	for (int i = 0; i < numLines; i++)
	{
		...
		double length = sqrt((__b.at<float>(0, 0) - __a.at<float>(0, 0)) * (__b.at<float>(0, 0) - __a.at<float>(0, 0)) + (__b.at<float>(1, 0) - __a.at<float>(1, 0)) * (__b.at<float>(1, 0) - __a.at<float>(1, 0)));
		sum_lengths += length;
