...
void MSAC::fillDataContainers(std::vector<std::vector<cv::Point>> &lineSegments) {
    ...
    for (int i = 0; i < numLines; i++) {
        ...
        double length = sqrt((__b.at<float>(0, 0) - __a.at<float>(0, 0)) *
    ...
    if (sum_lengths == 0) {
        __Lengths = __Lengths * ((double) 1 / 0.00000001);
    } else {
...
void MSAC::multipleVPEstimation(std::vector<std::vector<cv::Point>> &lineSegments,
    ...
    for (int vpNum = 0; vpNum < numVps; vpNum++) {
        ...
        while ((iter <= __min_iters) ||
               ((iter <= T_iter) && (iter <= __max_iters) && (no_updates <= max_no_updates))) {
            iter++;
            ...
            if (__Li.rows < (int) __MSS.size())
                break;
            GetMinimalSampleSet(__Li, __Lengths, __Mi, __MSS,
                                __vpAux); // output __vpAux is calibrated
                            ...
                            (unsigned int) __minimal_sample_set_dimension) // if J==0 maybe its because all line segments are perfectly parallel and the vanishing point is at the infinity
        {
            ...
            if (__mode == MODE_LS)
                ...
                estimateNIETO(__Li, __Lengths, __Mi, ind_CS, __N_I_best,
                              __vp); // Output __vp is calibrated
            else
...
// RANSAC
void MSAC::GetMinimalSampleSet(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &MSS,
                               cv::Mat &vp) {
    int N = Li.rows;
...
float MSAC::GetConsensusSet(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp,
                            std::vector<float> &E, int *CS_counter) {
...
void MSAC::estimateLS(cv::Mat &Li, cv::Mat &Lengths, std::vector<int> &set, int set_length,
                      cv::Mat &vp) {
...
void MSAC::estimateNIETO(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &set,
    ...
    if (__verbose) {
        printf("\nInitial Cal.VP = (%.3f,%.3f,%.3f)\n", vp.at<float>(0, 0), vp.at<float>(1, 0),
               vp.at<float>(2, 0));
        ...
        printf("Initial VP = (%.3f,%.3f,%.3f)\n", vpUnc.at<float>(0, 0), vpUnc.at<float>(1, 0),
               vpUnc.at<float>(2, 0));
    }
...
float MSAC::errorNIETO(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp,
                       std::vector<float> &E, int *CS_counter) {
    float J = 0;
    float di = 0;
...
void
MSAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point>>> &lineSegmentsClusters,
